...
- [ ] Tận dụng `boot_runner.py` + `startup_scan.py` làm chế độ quét định kỳ
- [ ] Trả payload thông báo có cấu trúc (số vấn đề mới, mức nghiêm trọng nhất)
- [ ] Read-only tuyệt đối: không tự xóa/move; expose hàm public ổn định cho UI gọi
...
- [ ] Với mỗi item trong selection, bổ sung field `reason_text` ngắn gọn tiếng Việt
      ("file tạm an toàn xóa" / "cần xem tay vì là project")
- [ ] Chỉ thêm dữ liệu, không đổi chữ ký hàm UI đang gọi
  - **Acceptance:** UI đọc `reason_text` hiển thị tooltip; tests pass.

### REQ#5 (tùy chọn) — Gom rule "file rác" tập trung
- [ ] Tài liệu hóa tiêu chí nào → `recommended_decision = delete_candidate`
- [ ] Đảm bảo không mở rộng quá tay (an toàn dữ liệu là số 1)
