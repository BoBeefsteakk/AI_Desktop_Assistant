...
- [ ] Thêm hàm public `build_one_click_cleanup_plan(...) -> dict` trả về
      `{files, count, total_size, summary_text, safety_contract}` từ scan gần nhất
- [ ] Chỉ gồm file `safe_delete` — **dùng `tools/core/cleanup_rules.py` làm nguồn chân lý**, không tự định nghĩa lại
- [ ] Read-only; KHÔNG đổi chữ ký hàm UI đang gọi (chỉ thêm hàm mới); ghi tên hàm + schema vào mailbox
- [ ] (Tùy chọn) thêm `severity` + `summary_text` tiếng Việt để UI hiện thẳng lên banner
