...
### **thay doi (2026-06-27) - Phase 7 backend REQ#1 den REQ#6**

* REQ#1: recommendation co `explanation` va advisor co `disk_full_reason`.
* REQ#2: Natural Command co `answer_user_question()` read-only.
* REQ#3: `run_periodic_scan()` tra notification schema co issue moi/severity,
  khong xoa/move/backup.
* REQ#4: moi selection item co `reason_text` cho tooltip UI.
* REQ#5: cleanup rule registry chi cho `safe_delete` thanh
  `delete_candidate`; boundary duoc test va tai lieu hoa.
* **thay doi REQ#6:** backend co `build_one_click_cleanup_plan()` de UI hien
  de nghi don 1 cham; plan van read-only va moi xoa that van qua decision report,
  dry-run, final token va Recycle Bin.
