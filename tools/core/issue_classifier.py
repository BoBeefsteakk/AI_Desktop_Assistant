...
def classify_large_file(file_item: dict[str, Any], *, index: int) -> dict[str, Any]:
    ...
    if cleanup_recommendation["recommended_decision"] == "keep":
        return make_issue(
            ...
            title="File lớn nằm trong vùng bảo vệ",
            detail="File này không được đưa vào luồng xóa/move tự động.",
    ...
    if cleanup_recommendation["recommended_decision"] == "delete_candidate":
        return make_issue(
            ...
            title="File có thể xóa an toàn nếu bạn chọn rõ",
            detail="Risk classifier đánh dấu safe_delete, nhưng adapter vẫn cần selection và token.",
            source=file_item.get("source", "storage"),
            path=path,
            size=size,
            risk=risk["risk"],
    ...
    if suffix in PROJECT_EXTENSIONS:
        return make_issue(
            ...
            title="File project/editing lớn",
            detail="Nên backup hoặc review kỹ, không xóa trực tiếp.",
            source=file_item.get("source", "storage"),
            path=path,
            size=size,
            risk=risk["risk"],
    ...
    if suffix in VIDEO_EXTENSIONS:
        return make_issue(
            ...
            title="Video lớn có thể gom/chuyển sau",
            detail="Phù hợp với move_later nếu bạn muốn gom media sang folder/drive riêng.",
            source=file_item.get("source", "storage"),
            path=path,
            size=size,
            risk=risk["risk"],
    ...
    if suffix in ARCHIVE_EXTENSIONS or suffix in INSTALLER_EXTENSIONS:
        return make_issue(
            ...
            title="File nén/bộ cài lớn cần review",
            detail="Chỉ xóa khi bạn chắc đã giải nén/cài xong hoặc đã backup.",
            source=file_item.get("source", "storage"),
            path=path,
            size=size,
            risk=risk["risk"],
    ...
    return make_issue(
        ...
        title="File lớn cần review",
        detail="Chưa đủ dữ kiện để xóa tự động; đưa vào danh sách review.",
        source=file_item.get("source", "storage"),
        path=path,
        size=size,
        risk=risk["risk"],
